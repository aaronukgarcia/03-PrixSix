...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (includeAll) {
      ...
      // Normal view: only pending emails ready to process (capped at 100 — GEMINI-AUDIT-119)
  ...
  try {
    ...
    if (action === 'push') {
      ...
      if (emailIds && emailIds.length > 0) {
        ...
        // Get pending emails — capped at PUSH_BATCH_LIMIT per invocation (GEMINI-AUDIT-119)
        const snapshot = await db.collection('email_queue')
          .where('status', '==', 'pending')
