...
export default function AdminPage() {
    ...
    // GUID: PAGE_ADMIN-HOTLINK-001-v03
    // [Intent] Admin Hot Link verification state management. Tracks whether the admin
    //          user has verified via magic link (adminVerified cookie check).
    // [Inbound Trigger] Page load and user auth changes.
    ...
    // GUID: PAGE_ADMIN-HOTLINK-002-v03
    // [Intent] Check for adminVerified cookie on page load. Cookie is set by
    //          /api/admin/verify-access after successful magic link verification.
    // [Inbound Trigger] Component mount or user changes.
    // [Downstream Impact] Sets adminVerified state which controls gate rendering.
    useEffect(() => {
        if (!user?.isAdmin) return;

        // Check for adminVerified cookie
        const cookies = document.cookie.split(';');
        const adminVerifiedCookie = cookies.find(c => c.trim().startsWith('adminVerified='));

        if (adminVerifiedCookie) {
            const value = adminVerifiedCookie.split('=')[1];
            setAdminVerified(value === 'true');
        } else {
