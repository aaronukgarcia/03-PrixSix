...
//   ownerId must equal request.auth.uid — since 'system' is never a real Firebase Auth UID,
//   no authenticated client can create a league with ownerId 'system'. This server-side check
...
// GUID: LIB_CONSISTENCY-027B-v02
// @SECURITY_FIX (GEMINI-AUDIT-050): Added input validation before parsing. Null/undefined/empty
//   breakdown strings return a safe zeroed default. A regex pre-check validates that the string
//   contains at least one valid "Name+N" token before splitting; malformed strings are logged
//   as a warning (with correlationId for traceability) and return the safe default rather than
//   silently producing incorrect scores or throwing.
...
//   Returns a safe zeroed default for null/undefined/empty/malformed input.
...
//   Malformed breakdown strings now produce a warning issue rather than silent data corruption.
...
// Minimum format validation: string must contain at least one "Word+Digit" token.
// This catches completely malformed strings (e.g., garbage data, truncated writes) before
// the comma-split loop runs. Does not fully validate all tokens — per-token validation
// happens inside the loop via the individual match() call.
const BREAKDOWN_MIN_FORMAT_REGEX = /\S+\+\d+/;

function parseBreakdown(
  breakdown: string | null | undefined,
  driverNameToId: Map<string, string>,
  correlationId?: string
): { entries: BreakdownEntry[]; bonusPoints: number; sum: number; malformed: boolean } {
  const safeDefault = { entries: [] as BreakdownEntry[], bonusPoints: 0, sum: 0, malformed: false };

  // Guard: null / undefined / empty string — not an error, just no breakdown data
  if (breakdown === null || breakdown === undefined || breakdown.trim() === '') {
    return safeDefault;
  }

  // Guard: format pre-check — must contain at least one "Token+N" pattern
  if (!BREAKDOWN_MIN_FORMAT_REGEX.test(breakdown)) {
    // Log warning: malformed breakdown will not be parsed (GEMINI-AUDIT-050)
    // correlationId included for support traceability
    console.warn(
      `[consistency] parseBreakdown: malformed breakdown string, skipping parse` +
      (correlationId ? ` [correlationId: ${correlationId}]` : ''),
      { breakdown: breakdown.substring(0, 200) }  // Truncate to avoid log flooding
    );
    return { ...safeDefault, malformed: true };
  }
  ...
  for (const part of parts) {
    if (!part) continue;  // Skip empty tokens from trailing commas or double-commas
    const match = part.match(/^(.+?)\+(\d+)$/);
    if (!match) continue;  // Skip tokens that don't match expected format — tolerate partial corruption
    ...
    if (score.raceId && score.userId && score.totalPoints !== undefined) {
      ...
      if (matchingPredictions.length > 0 && raceResult) {
        ...
        if (actualTop6.length === 6) {
          ...
          if (score.breakdown) {
            // Pass score.id as correlationId so malformed-breakdown warnings are traceable
            const parsed = parseBreakdown(score.breakdown, driverNameToId, score.id);

            // GEMINI-AUDIT-050: If breakdown string was malformed, surface a warning issue
            // and skip the remaining breakdown checks (no reliable data to verify against).
            if (parsed.malformed) {
              issues.push({
                severity: 'warning',
                entity: entityName,
                field: 'breakdown',
                message: `Malformed breakdown string could not be parsed — score type analysis skipped for this score`,
                details: { breakdown: score.breakdown?.substring(0, 200) },
              });
              // Do not set isValid = false — a malformed breakdown is a data quality warning,
              // not a score calculation error. The score's totalPoints may still be correct;
              // we just cannot verify it via breakdown cross-check.
            } else {
            ...
            } // end else (breakdown not malformed)
          }
        }
      }
    }
...
// GUID: LIB_CONSISTENCY-030-v05
// @SECURITY_FIX (GEMINI-AUDIT-049): All 'system' string comparisons replaced with SYSTEM_OWNER_ID constant.
// @SECURITY_FIX (ZfdQM8upCSadgj5Pk4cF): Added guard that flags any league with ownerId === SYSTEM_OWNER_ID
//   but isGlobal !== true as a security anomaly. Defense-in-depth comment documents the Firestore rule
//   enforcement layer (firestore.rules line 245: CREATE requires ownerId === request.auth.uid, which
//   prevents any authenticated client from writing ownerId: 'system'). This consistency check is
//   server-side validation that catches data integrity violations not preventable by client-facing rules
//   (e.g., direct Admin SDK writes that omit the isGlobal flag).
...
//   - SYSTEM_OWNER_ID leagues must have isGlobal === true (anomaly detection)
...
export function checkLeagues(
  ...
  for (const league of leagues) {
    ...
    if (!league.ownerId) {
      ...
      isValid = false;
    } else if (league.ownerId === SYSTEM_OWNER_ID && league.isGlobal !== true) {
      // SECURITY (ZfdQM8upCSadgj5Pk4cF): League claims system ownership but is not marked isGlobal.
      // Firestore rules (firestore.rules line 245) prevent authenticated clients from creating a
      // league with ownerId 'system' (CREATE enforces ownerId === request.auth.uid). This anomaly
      // indicates a data integrity issue: either a migration error or an Admin SDK write that
      // omitted the isGlobal flag. Flag as error to surface for admin investigation.
      issues.push({
        severity: 'error',
        entity: entityName,
        field: 'ownerId',
        message: `Security anomaly: ownerId is SYSTEM_OWNER_ID ('system') but isGlobal is not true — this league should not exist in this state`,
        details: { ownerId: league.ownerId, isGlobal: league.isGlobal },
      });
      isValid = false;
