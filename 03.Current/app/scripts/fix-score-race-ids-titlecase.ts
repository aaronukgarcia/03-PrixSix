...
async function fixScoreRaceIds() {
  console.log('\nðŸ”§ FIX SCORE RACE IDS TO TITLE-CASE');
  console.log(`Mode: ${DRY_RUN ? 'DRY RUN' : 'ðŸŸ¢ LIVE UPDATE'}\n`);

  const scoresSnapshot = await db.collection('scores').get();
  console.log(`ðŸ“Š Found ${scoresSnapshot.size} scores to check\n`);
  ...
  scoresSnapshot.forEach(doc => {
    ...
    if (!currentRaceId) {
      console.log(`âš ï¸  Skipping ${doc.id} - no raceId`);
      return;
    }
    ...
    if (currentRaceId !== titleCaseRaceId) {
      needsUpdate++;
      if (needsUpdate <= 5) {
        console.log(`  ${currentRaceId} â†’ ${titleCaseRaceId}`);
      }
  ...
  console.log(`\nðŸ“Š Summary: ${needsUpdate} scores need Title-Case update\n`);
  ...
  if (needsUpdate === 0) {
    console.log('âœ… All scores already in Title-Case!\n');
    return;
  }

  console.log('ðŸ’¾ Updating scores...');
  await batch.commit();
  console.log(`âœ… Updated ${needsUpdate} scores to Title-Case!\n`);
}
