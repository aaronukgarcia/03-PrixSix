#!/usr/bin/env tsx
/**
 * Send prediction reminder email to users with no predictions.
 *
 * GUID: SCRIPT_SEND_PREDICTION_REMINDER-000-v01
 * [Intent] Send a professional, branded reminder email to registered users who haven't submitted predictions.
 * [Inbound Trigger] Manual execution to remind inactive users about the upcoming season.
 * [Downstream Impact] Sends transactional email via Microsoft Graph API.
 */
...
// Now import email module (after env vars are loaded)
import { sendEmail } from '../app/src/lib/email';
...
async function main() {
  ...
  const recipientEmail = 'aaron@garica.ltd';
...
<body>
  ...
  try {
    ...
    console.log(`   Email GUID: ${result.guid}`);
