...
async function showUserPrediction() {
  ...
  // Get all predictions and filter locally
  ...
  if (!australianGPPrediction) {
    ...
    console.log('Full prediction document:');
    console.log('─'.repeat(60));
    console.log(JSON.stringify(pred, null, 2));
    console.log('');
  }

  console.log('All predictions for this user:');
  console.log('─'.repeat(60));
  allPredictions.docs.forEach(doc => {
    const pred = doc.data();
    console.log(`  - ${pred.raceId} (positions: ${pred.positions?.length || 0})`);
  });
