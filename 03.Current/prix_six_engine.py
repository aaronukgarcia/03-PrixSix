...
import bleach  # For sanitizing AI-generated HTML (GEMINI-003 fix)
import feedparser
...
# ---------------------------------------------------------------------------
# HTML Sanitization (GEMINI-003 Fix)
# ---------------------------------------------------------------------------

# Allowed HTML tags and attributes for AI-generated newsletter content
# This whitelist prevents XSS from adversarial AI inputs (e.g., malicious RSS feeds)
ALLOWED_TAGS = [
    'p', 'h1', 'h2', 'h3', 'h4', 'strong', 'em', 'b', 'i', 'u',
    'ul', 'ol', 'li', 'br', 'hr', 'a', 'span', 'div', 'blockquote'
]
...
def sanitize_ai_html(unsafe_html: str) -> str:
    ...
    Defense-in-depth: Even though the AI is prompted to generate safe HTML,
    adversarial inputs (e.g., malicious RSS feed content) could inject XSS
    payloads that the AI might unknowingly include in its output.

    Args:
        unsafe_html: Raw HTML generated by Gemini 1.5 Pro

    Returns:
        Sanitized HTML with only allowed tags and attributes

    Security: GEMINI-003 fix - prevents stored XSS in newsletter content
    """
    return bleach.clean(
        unsafe_html,
        tags=ALLOWED_TAGS,
...
def build_html(body_html: str) -> str:
    ...
    # Sanitize AI-generated content before insertion (GEMINI-003 fix)
    safe_body_html = sanitize_ai_html(body_html)
    return textwrap.dedent(f"""\
        <!DOCTYPE html>
        <html lang="en">
        <head>
...
def write_to_firestore(body_html: str) -> None:
    ...
    Security: GEMINI-003 fix - sanitizes AI-generated HTML before storing in Firestore.
    """
    try:
        ...
        # Sanitize AI-generated content before storing (GEMINI-003 fix)
        safe_body_html = sanitize_ai_html(body_html)

        db = admin_firestore.client()
        db.collection("app-settings").document("pub-chat").set(
            {
                "content": safe_body_html,
